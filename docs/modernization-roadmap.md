...
- Filtering modes are tested against promoted `standard_12.adaptECG`; the normal male bundle uses derived P/T baseline samples from that export, while decoded `.ECGsimcase` fiducial fields remain unavailable.
- Initial/adapted Thorax BSPM, sensitivity-map, and adapted lead-trace previews recompute for a shape-matched transfer candidate, but WCT/reference lead-transform parity, BSPM numerical parity, and contribution maps remain incomplete.
