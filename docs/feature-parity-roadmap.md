...
- `0052` Implement Fiducial And Filtering Parity: completed for the promoted normal male `standard_12.adaptECG` export using inferred zero-run baseline samples; task `0068` promotes those samples into normal-male case metadata, while decoded P/T fiducial semantics still need resolution for arbitrary `.ECGsimcase` inputs.
