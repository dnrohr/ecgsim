...
| GW-011 | Create or edit focus/activation | WPW cases | Open Focus tools, inspect or edit activation focus, opposite wall, and propagation velocity where supported. | Focus data is visible/editable; activation/source changes propagate to TMP and ECG after recompute. | Parser tests for activation/focus data plus graph-solver tests and future recomputation parity test. | Activation payloads and route solver exist; interactive focus tools and recompute propagation remain missing. |
| GW-012 | Export and reload adapted work | `normal_male2.ECGsimcase` | Make a small TMP edit, export directory, save sidecar/case, reload output, and compare values. | Exported ECG/TMP/source/electrode/triangulation files are present; reloaded adaptation preserves edits. | File-level export regression tests, sidecar browser workflow tests, capture validation tools, and future MATLAB/readECGsim compatibility smoke where possible. | Supported export subset and sidecar exist; TMP `.user.source`, electrodes, adapted ECG recomputation, and full legacy export parity remain missing. |
