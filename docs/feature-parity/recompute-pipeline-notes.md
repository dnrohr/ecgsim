...
- Task `0049` now has a promoted normal male ECGSIM 3.0.1 `.user.source` fixture, but TMP generator parity has not yet been implemented, so the TMP generator remains a deterministic preview rather than a parity-tested source matrix generator.
...
1. Use the promoted normal male ECGSIM 3.0.1 `.user.source` fixture to establish TMP generator parity.
