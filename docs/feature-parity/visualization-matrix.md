...
| Leads | Measured/initial/adapted overlays | Case traces plus adapted recompute preview; measured/initial disabled with status text | blocked-on-evidence | Lead/reference/display indices are parsed from payloads; reference-weight equations and measured/initial classification still need decoded semantics | future |
