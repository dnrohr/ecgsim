...
| Heart | Endocardial/epicardial and transmural mapping | Computed Source wall depth surface plus parsed `PGraphGeometry` source-mesh overlay; exact wall-pair controls remain disabled | modern-equivalent | Source mesh is visible, source-node aligned, and now exposes a normalized radial wall-depth scalar; explicit legacy wall-pairing semantics remain undecoded | complete |
...
| TMP | Electrogram | EGM toggle draws a computed selected-node preview from dense source-to-source transfer candidate and generated TMP | modern-equivalent | Case metadata verifies one dense signed transfer candidate and TMP fixtures carry the matrix for computed preview; exact VENTR.VENTRICLES role and output scale still need legacy validation | complete |
...
| Leads | Measured/initial/adapted overlays | Promoted or imported measured ECG traces plus initial/adapted recompute overlays; unavailable measured cases show explicit status text | modern-equivalent | Visual workflow is data-backed by promoted `.refECG` exports, external ECG import, and computed initial/adapted traces; arbitrary case-payload classification and final reference weights remain numerical parity work | complete |
