...
| Leads | Measured/initial/adapted overlays | Case traces plus adapted recompute preview; measured/initial disabled with status text | blocked-on-evidence | PMatrix roles and lead/reference labels inventoried; standard/BSPM WCT/reference equations and measured/initial classification still need decoded trailing-field semantics | future |
