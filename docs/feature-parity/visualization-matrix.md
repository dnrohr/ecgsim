...
| Leads | Measured/initial/adapted overlays | Case traces plus adapted recompute preview; measured/initial disabled with status text | blocked-on-evidence | Lead labels are parsed from payloads; standard/BSPM WCT/reference equations and measured/initial classification still need decoded trailing-field semantics | future |
