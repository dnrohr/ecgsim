...
| Heart | Source mesh overlay and computed wall depth | App workflow toggles the parsed `PGraphGeometry` source mesh, selects Source wall depth, and verifies canvas deltas; visual-mode smoke covers the wall-depth surface; smoke tests pin source-mesh fixture shape and normalized wall-depth values | medium | Exact legacy endocardial/epicardial pairings and transmural grouping semantics remain unavailable |
...
| TMP | Electrogram preview | App workflow enables EGM, verifies matrix-index evidence, and checks TMP canvas redraw; regression tests pin zero source-node-by-time candidates, one dense signed transfer candidate, and fixture-carried transfer matrix dimensions | medium | Needs confirmed VENTR.VENTRICLES role and legacy-validated selected-node EGM output scale |
...
| Leads | Measured/initial/adapted overlays | App workflow toggles promoted normal-male measured `.refECG`, imported ECG, initial, adapted, and initial+adapted overlays; smoke tests cover measured export fixture metadata and recompute dimensions | medium | Arbitrary case-payload measured classification and final lead transform/WCT parity remain numerical parity work |
