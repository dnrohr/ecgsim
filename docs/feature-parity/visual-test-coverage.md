...
| TMP | Electrogram blocker | App workflow verifies EGM control is disabled and names matrix-inventory evidence; regression tests pin zero source-node-by-time candidates | blocker evidence | Needs selected-node electrogram payload or derivation equation |
