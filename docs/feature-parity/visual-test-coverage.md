...
| TMP | Electrogram blocker | App workflow verifies EGM control is disabled and names matrix-inventory/source-square evidence; regression tests pin zero source-node-by-time candidates and one dense signed transfer candidate | blocker evidence | Needs confirmed VENTR.VENTRICLES role and legacy-validated selected-node EGM output |
