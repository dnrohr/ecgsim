...
- Adapted lead traces in the browser are recomputed from edited TMP source parameters at parsed electrode thorax nodes, then passed through the existing coupling filter modes. They are intentionally labeled as unresolved for lead reference-weight parity until the final weighting equations are decoded.
