...
- Heart and Thorax Contours are scalar-node overlays near evenly spaced levels. They are useful inspection aids, not exact interpolated legacy isofunction lines.
- Heart Thorax contribution mode displays the selected Thorax node's ventricles-to-thorax transfer row. Electrode target shortcuts, lead-transfer contribution roles, and WCT/reference parity remain unresolved.
...
- The Heart Source mesh overlay shows the parsed `PGraphGeometry` source topology. Source wall depth shows a computed normalized wall-depth preview from that mesh. Endocardial/epicardial and transmural controls are still disabled until explicit wall pairings are parsed.
- Show EGM adds a computed selected-node electrogram preview from the dense signed source-to-source transfer candidate and generated TMP waveforms. Treat the scale as preview evidence until the exact legacy `VENTR.VENTRICLES` role is validated.
